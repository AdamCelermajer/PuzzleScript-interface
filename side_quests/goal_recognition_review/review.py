...
SIDE_QUESTS_DIR = Path(__file__).resolve().parents[1]
ARC_BACKEND_URL = "https://three.arcprize.org"
PUZZLESCRIPT_BACKEND_URL = "http://127.0.0.1:8000"
PUZZLESCRIPT_SETUPS = {"pz_one_frame", "pz_ten_frame_random"}
...
def default_evidence_roots(setup: str) -> list[Path]:
    if setup == "one_frame":
        return [SIDE_QUESTS_DIR / "one_frame_goal_recognition" / "artifacts"]
    if setup == "two_frame":
        return [SIDE_QUESTS_DIR / "two_frame_goal_recognition" / "artifacts"]
    if setup == "ten_frame_random":
        return [SIDE_QUESTS_DIR / "ten_frame_goal_recognition" / "artifacts"]
    if setup == "pz_one_frame":
        return [SIDE_QUESTS_DIR.parent / "pz_sideproject1" / "artifacts"]
    if setup == "pz_ten_frame_random":
        return [SIDE_QUESTS_DIR.parent / "pz_sideproject10" / "artifacts"]
    return [SIDE_QUESTS_DIR]
