...
from .model import HelpedFrame, HelpedGoal
from .perceiver import HelpedPerceiver
from .rules import DEFAULT_OUTPUT_PATH, HelpedRuleModel
...
PLANNING_NODE_LIMIT = 200


class HelpedEnv(Protocol):
...
class HelpedRunResult:
    goal_reached: bool
    steps: int
    actions: list[GameAction]
    final_frame: HelpedFrame
    output_path: Path


class HelpedLiveRunner:
    def __init__(
        self,
        env: HelpedEnv,
        goal: HelpedGoal,
        ...
        model: HelpedRuleModel | None = None,
        max_steps: int = 40,
    ...
    def _choose_action(self, current: HelpedFrame, frame_data: FrameData) -> GameAction:
    ...
    def _known_plan_to_goal(
        self, current: HelpedFrame, frame_data: FrameData
    ) -> list[GameAction] | None:
        actions = self._available_actions(frame_data)
        queue: deque[tuple[HelpedFrame, list[GameAction]]] = deque([(current, [])])
        seen = {current}
        nodes = 0
        ...
        while queue:
            ...
            nodes += 1
            if nodes > PLANNING_NODE_LIMIT:
                self._event("goal planning node budget reached")
                return None
    ...
    def _plan_to_unseen_context(
        ...
        while queue:
            ...
            nodes += 1
            if nodes > PLANNING_NODE_LIMIT:
                self._event("reachable-context planning node budget reached")
                return None
    ...
    def _mark_attempt(self, frame: HelpedFrame, action: GameAction) -> None:
        key = (frame, action)
        self.action_attempts[key] = self.action_attempts.get(key, 0) + 1

    def _finish(
        self, reached: bool, steps: int, current: HelpedFrame
    ) -> HelpedRunResult:
    ...
    def _write_report(
        self, current: HelpedFrame, final: bool, reached: bool | None = None
    ) -> None:
        sections = [
            "## Goal Facts",
            "",
            ...
            "",
            "## Current Layered Frame",
            "",
            *[f"- `{row}`" for row in current.to_rows()],
            "",
            "## Current Helped Facts",
            "",
