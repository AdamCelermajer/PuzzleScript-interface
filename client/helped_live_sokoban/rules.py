...
DEFAULT_OUTPUT_PATH = (
    Path(__file__).resolve().parent / "output" / "helped_live_rules.md"
)
DEFAULT_STORE_PATH = (
    Path(__file__).resolve().parent / "output" / "helped_live_rules.json"
)
DEFAULT_JOURNAL_PATH = (
    Path(__file__).resolve().parent / "output" / "helped_live_journal.md"
)
...
class HelpedLineRule:
    ...
    def predictions(self, frame: HelpedFrame) -> list[HelpedFrame]:
        predictions = []
...
class HelpedRuleModel:
    """Learns action effects over PuzzleScript symbol lines."""
    ...
    def __init__(
        ...
        self.rules: list[HelpedLineRule] = []
        self.failures: list[HelpedFailure] = []
        self._next_id = 1
    ...
    def observe(
        self, before: HelpedFrame, action: GameAction | str, after: HelpedFrame
    ) -> None:
    ...
    def predict(self, before: HelpedFrame, action: GameAction | str) -> list[HelpedFrame]:
        action_name = _action_name(action)
        predictions: list[HelpedFrame] = []
        seen: set[HelpedFrame] = set()
        for rule in self.active_rules(action_name):
            for predicted in rule.predictions(before):
                if predicted not in seen:
                    predictions.append(predicted)
                    seen.add(predicted)
        for rule in self._generalized_source_rules(action_name):
            for predicted in self._generalized_predictions(rule, before, action_name):
                if predicted not in seen:
                    predictions.append(predicted)
                    seen.add(predicted)
        return predictions
    ...
    def record_prediction_result(
        ...
        generalized_matching = [
            rule
            for rule in self._generalized_source_rules(action_name)
            if after in self._generalized_predictions(rule, before, action_name)
        ]
        if matching or generalized_matching:
            ...
            self._append_journal(
                ...
                f"generalized_from={tuple(rule.id for rule in generalized_matching)}"
            )
            self.write()
            return

        expected_ids = self._predictor_ids_for_predictions(
            before, action_name, predictions
        )
        failure = HelpedFailure(
            action=action_name,
    ...
    def active_rules(self, action: str | None = None) -> list[HelpedLineRule]:
        return [
            rule
            for rule in self.rules
    ...
    def action_has_delta(self, action: GameAction) -> bool:
        action_name = _action_name(action)
        return any(
            known_action == action_name
            for known_action, _symbol in self.action_deltas
        )

    def unseen_context_action(
        self, frame: HelpedFrame, actions: list[GameAction]
                ...
                for x, y in frame.positions(symbol):
                    for length in (3, 2):
                        ...
                        if before_values is None:
                            continue
                        context = (symbol, action_name, dx, dy, before_values)
    ...
    def _learn_action_deltas(
        self, before: HelpedFrame, action: str, after: HelpedFrame
    ) -> list[tuple[str, int, int]]:
        learned = []
        movements = self._occupant_movements(before, after)
        if len(movements) != 1:
            return learned

        symbol, old_x, old_y, new_x, new_y = movements[0]
        dx = new_x - old_x
        dy = new_y - old_y
        if abs(dx) + abs(dy) != 1:
            return learned
    ...
    def _occupant_movements(
        self, before: HelpedFrame, after: HelpedFrame
    ) -> list[tuple[str, int, int, int, int]]:
        movements = []
        symbols = set(before.occupant_symbols()) | set(after.occupant_symbols())
        for symbol in sorted(symbols):
            ...
            for old_x, old_y in vanished:
                for new_x, new_y in emerged:
                    movements.append((symbol, old_x, old_y, new_x, new_y))
        return movements
    ...
    def _line_rule_from_transition(
        ...
        for x, y in anchors:
            ...
            if before_line == after_line and changed:
                continue
            return HelpedLineRule(
                id=self._new_rule_id(),
                action=action,
                anchor_symbol=anchor_symbol,
                dx=dx,
                dy=dy,
                before_values=before_line,
                after_values=after_line,
            )
        return None
    ...
    def _find_matching_rule(self, candidate: HelpedLineRule) -> HelpedLineRule | None:
        for rule in self.rules:
            if (
    ...
    def _generalized_source_rules(self, action: str) -> list[HelpedLineRule]:
        return [
            rule
            for rule in self.active_rules()
            if rule.action != action
            and (action, rule.anchor_symbol) in self.action_deltas
        ]

    def _generalized_predictions(
        self, rule: HelpedLineRule, frame: HelpedFrame, action: str
    ) -> list[HelpedFrame]:
        delta = self.action_deltas.get((action, rule.anchor_symbol))
        if delta is None:
            return []

        dx, dy = delta
        predictions = []
        for x, y in frame.positions(rule.anchor_symbol):
            before_line = frame.line(x, y, dx, dy, len(rule.before_values))
            if before_line != rule.before_values:
                continue
            predicted = frame.apply_line(x, y, dx, dy, rule.after_values)
            if predicted is not None:
                predictions.append(predicted)
        return predictions

    def _predictor_ids_for_predictions(
        self,
        before: HelpedFrame,
        action: str,
        predictions: list[HelpedFrame],
        ...
        for rule in self._generalized_source_rules(action):
            if any(
                predicted in predictions
                for predicted in self._generalized_predictions(rule, before, action)
            ):
                expected_ids.append(f"{rule.id}->{action}")
        return tuple(expected_ids)
    ...
    def _context_observations(self, context: tuple) -> int:
        symbol, action, dx, dy, before_values = context
        return self.context_attempts.get(
            self._context_key(context), 0
        ) + self._line_observations(
            str(action),
            str(symbol),
            int(dx),
            int(dy),
    ...
    def _load(self) -> None:
        ...
        if version != 5:
            self._append_journal(
                f"ignored incompatible helped rule store version {version}; "
                "symbol semantics changed"
            )
            return
        ...
        self.rules = [
            HelpedLineRule(
                ...
                dx=int(item["dx"]),
                dy=int(item["dy"]),
                before_values=tuple(str(value) for value in item["before_values"]),
                after_values=tuple(str(value) for value in item["after_values"]),
        ...
        self._append_journal(
            f"loaded {self.loaded_rule_count} persisted helped rules from {self.store_path}"
        )
    ...
    def _cross_action_generalizations(
        self,
    ) -> list[tuple[HelpedLineRule, str, int, int]]:
        candidates = []
        for rule in self.active_rules():
            for (action, symbol), (dx, dy) in sorted(self.action_deltas.items()):
                if action == rule.action or symbol != rule.anchor_symbol:
                    continue
                candidates.append((rule, action, dx, dy))
        return candidates
    ...
    def _append_journal(self, event: str) -> None:
        ...
        if not self.journal_path.exists():
            self.journal_path.write_text(
                "# Helped LIVE Rule Growth Journal\n\n",
                encoding="utf-8",
            )
    ...
    def _render(self, final: bool, extra_sections: list[str]) -> str:
        lines = [
            "# Helped LIVE Sokoban",
            "",
            "State uses a PuzzleScript symbol representation.",
            "Actions remain opaque names such as ACTION1; no direction labels are supplied.",
            "The model receives no object role names and no transition semantics.",
        ...
        lines.extend(["", "## Cross-Action Generalizations", ""])
        generalizations = self._cross_action_generalizations()
        if not generalizations:
            lines.append("- None yet.")
        for rule, target_action, target_dx, target_dy in generalizations:
            lines.extend(
                [
                    f"### {rule.id} -> {target_action}",
                    "",
                    f"- source_rule: {rule.id}",
                    f"- source_action: {rule.action}",
                    f"- target_action: {target_action}",
                    f"- anchor_symbol: {rule.anchor_symbol}",
                    f"- target_delta: ({target_dx},{target_dy})",
        ...
        for index, failure in enumerate(self.failures, start=1):
            lines.append(
                ...
                f"predictors={failure.expected_rule_ids}"
            )

        lines.extend(extra_sections)
        if final:
            lines.extend(["", "## Final Rule Set", "", "Final snapshot written after run."])
        return "\n".join(lines) + "\n"
