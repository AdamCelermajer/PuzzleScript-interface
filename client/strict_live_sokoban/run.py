...
from client.strict_live_sokoban.model import RawGoal
from client.strict_live_sokoban.runner import StrictLiveRunner
from client.strict_live_sokoban.rules import (
...
def load_goal(path: Path) -> RawGoal:
    data = json.loads(path.read_text(encoding="utf-8"))
    return RawGoal.from_cells(data["required_cells"])


def main() -> None:
    parser = ArgumentParser(
        description="Strict raw-state parameterless LIVE-style experiment"
    )
    ...
    if not args.no_dashboard:
        dashboard = TerminalDashboard(
            ...
            mode="STRICT-LIVE",
            controls="Raw parameterless experiment. Press Ctrl+C to stop.",
    ...
    try:
        ...
        for episode in range(1, max(1, args.episodes) + 1):
            event_sink(f"episode {episode}/{max(1, args.episodes)}")
