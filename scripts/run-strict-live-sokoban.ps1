...
$OutputDir = Join-Path $Root "client\strict_live_sokoban\output"
...
try {
    if (-not (Test-Port -Port $RuntimePort)) {
        ...
        $runtimeLog = Join-Path $OutputDir "strict-runtime.log"
        $runtimeErr = Join-Path $OutputDir "strict-runtime.err.log"
    ...
    if (-not (Test-Port -Port $BackendPort)) {
        ...
        $apiLog = Join-Path $OutputDir "strict-arc-adapter.log"
        $apiErr = Join-Path $OutputDir "strict-arc-adapter.err.log"
    ...
    Write-Host "Running strict parameterless LIVE experiment for $Episodes episode(s)..."
    $clientArgs = @(
        "-m",
        "client.strict_live_sokoban.run",
        "--backend-url",
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Strict LIVE run exited with code $LASTEXITCODE."
    }
}
finally {
