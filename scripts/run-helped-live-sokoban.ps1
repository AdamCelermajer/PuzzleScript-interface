...
$OutputDir = Join-Path $Root "client\helped_live_sokoban\output"
...
try {
    if (-not (Test-Port -Port $RuntimePort)) {
        ...
        $runtimeLog = Join-Path $OutputDir "helped-runtime.log"
        $runtimeErr = Join-Path $OutputDir "helped-runtime.err.log"
    ...
    if (-not (Test-Port -Port $BackendPort)) {
        ...
        $apiLog = Join-Path $OutputDir "helped-arc-adapter.log"
        $apiErr = Join-Path $OutputDir "helped-arc-adapter.err.log"
    ...
    Write-Host "Running helped LIVE experiment for $Episodes episode(s)..."
    $clientArgs = @(
        "-m",
        "client.helped_live_sokoban.run",
        "--backend-url",
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Helped LIVE run exited with code $LASTEXITCODE."
    }
}
finally {
