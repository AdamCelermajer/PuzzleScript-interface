...
from puzzlescript_arc.app import create_app  # type: ignore[import-not-found]
from puzzlescript_arc.catalog import GameCatalogEntry  # type: ignore[import-not-found]
...
class ArcServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        self.catalog = [
            GameCatalogEntry(
                ...
                file_path=os.path.join(ROOT, "games", "sokoban-basic.txt"),
            ),
            GameCatalogEntry(
                game_id="transition-v1",
                title="transition",
                source_name="transition",
                file_path=os.path.join(ROOT, "games", "transition.txt"),
            ),
        ]
    ...
    def test_game_over_runs_do_not_count_as_completed(self) -> None:
        ...
        scorecard = self.client.get(f"/api/scorecard/{card_id}").json()
        self.assertEqual(scorecard["total_environments_completed"], 0)
        self.assertFalse(scorecard["environments"][0]["completed"])
        self.assertFalse(scorecard["environments"][0]["runs"][0]["completed"])
