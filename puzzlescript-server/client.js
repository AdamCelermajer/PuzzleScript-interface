...
class PuzzleScriptClient {
    ...
    displayBoard(result) {
        ...
        console.log('\n' + result.board + '\n');
        console.log('='.repeat(60) + '\n');
        console.log('Controls: WASD to move, R to reset, Q to quit');
    }
...
async function main() {
    ...
    try {
        ...
        const loop = () => {
            rl.question('Action (WASD): ', async (ans) => {
                ...
                if (['w', 'a', 's', 'd', 'r', 'reset'].includes(cmd)) {
                    try {
                        const result = await client.action(cmd);
                        client.displayBoard(result);
                        client.displayLegend(currentLegend); // Keep displaying legend
